...
- Current closeout gate: this PR body intentionally does not name a latest closeout head, because each evidence-only commit supersedes that value. Before merge or planning acceptance, verify live PR #9 with `gh pr view 9 --json headRefOid,statusCheckRollup,reviews,comments`, require both governance checks to pass on that live head, and require Codex no-major for that live head.
