...
| Functionality | PASS locally | OpenAlex, Crossref, Semantic Scholar, arXiv, and user-upload metadata normalizers emit Stage 02-compatible `SourceCreate`, `DocumentCreate`, and `ToolCallLogCreate` payloads with provider metadata, locator/DOI/URL, retrieval time, publication time, transformation notes, validation status, and sanitized safe arguments. |
| Tests | PASS locally | `python -m pytest apps/api/tests/test_stage03_connectors.py -q` passed with 14 tests; `python -m pytest apps/api/tests -q --maxfail=1` passed with 67 tests after updating the Stage 02 forbidden-scope guard for the now-authorized Stage 03 connector package; compileall, phase check, no-network import scan, forbidden Stage 04+ artifact scan, and high-confidence secret scan passed. |
