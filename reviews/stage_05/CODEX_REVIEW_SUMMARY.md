...
| CR-05-022: current-state route still told a pushed head to commit again | https://github.com/xiaoming2cf-afk/FinSignalHub/pull/12#discussion_r3380897791 | remediated in current-state route wording | `CONTROL/24_CURRENT_STAGE_STATE.md` now uses the dirty/clean-not-on-PR/live-head state machine. Pushed head `540aba997c0890938c0d316771cc4d012b2f0b8b` passed CI and received current-head Codex review. |
| CR-05-023: B-0126 blocker row still told a clean pushed head to commit again | https://github.com/xiaoming2cf-afk/FinSignalHub/pull/12#discussion_r3380922178 | locally remediated; external recheck pending | `CONTROL/20_BLOCKER_LOG.md` B-0126 now uses the same dirty/clean-not-on-PR/live-head state machine: local edits require one checked commit, clean local HEAD not on PR requires pushing the checked head, and PR head equal to local HEAD requires live CI/Codex/thread evidence without another status commit. |
