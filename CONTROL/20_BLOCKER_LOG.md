...
| B-0025 | 2026-05-29T17:46:59-05:00 | 02 | Docker Compose Postgres user mismatch | open | Codex reviewed head `6bff2191781b02d6e2bb2459a3c1efae05bfedf2` and returned CR-02-042 because `docker-compose.yml` allowed `POSTGRES_USER` override while the API database URL hard-coded `finsignalhub`. | PR #8 cannot merge until the API compose URL honors `POSTGRES_USER`, local compose/API checks pass, the next pushed head passes CI/Codex, and GPT Pro delta/final re-review accepts the latest remediation. | Update `FINSIGNALHUB_DATABASE_URL` to use `${POSTGRES_USER:-finsignalhub}`, run compose config and API checks, push, wait for CI, request follow-up Codex, then submit GPT Pro delta/final re-review. | no | Codex |
