...
| B-0024 | 2026-05-29T17:24:13-05:00 | 02 | Nullable dependent delete provenance stripping | open | Codex reviewed head `52a99629b5f2cf136e39efc1e4d4b47858abfe47` and returned CR-02-041 because IntegrityError-only delete handling can miss nullable one-to-many provenance references when SQLAlchemy nulls child FKs before commit. | PR #8 cannot merge until delete paths pre-check dependent rows, local tests pass, the next pushed head passes CI/Codex, and GPT Pro delta/final re-review accepts the latest remediation. | Add pre-delete ONETOMANY dependent checks, add a ToolCallLog provenance-preservation regression test, run checks, push, wait for CI, request follow-up Codex, then submit GPT Pro delta/final re-review. | no | Codex |
