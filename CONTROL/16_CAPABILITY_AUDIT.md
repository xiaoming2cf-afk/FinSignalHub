...
| Computer Use local plugin draft | governance-only wrapper created locally | document safe recovery route and reduce repeated failed attempts | Local files can define skill and plugin instructions, but they cannot expose a missing host tool. | Future runs have a clear safe route, but Stage 05 GPT Pro Gate 7 still needs a real callable Computer Use/Chrome route and a safe page state. | Use `.agents/skills/computer-use-gpt-pro-reviewer/SKILL.md`; do not claim capability availability from the manifest alone. | no for documentation; yes for actual browser interaction |
