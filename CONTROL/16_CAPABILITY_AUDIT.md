...
| Chrome extension | available for Stage 03 GPT Pro follow-up through logged-in DOM route | GPT Pro page and Codex settings pages with login state | Earlier extension routes returned `native pipe is closed` or timed out on screenshot/page-state calls, but the later logged-in Chrome extension DOM route claimed the existing GPT Pro tab, submitted the follow-up packet, and saved the PASS response without entering secrets or disturbing the foreground tab. | Stage 03 GPT Pro planning review can be claimed from the saved Chrome extension evidence; future reviews still require readable DOM and no login/permission/secret prompt. | Use the successful DOM route when stable; otherwise record a blocker or wait for standalone background Computer Use instead of using foreground automation. | no for the saved follow-up; yes for future foreground-only fallback |
...
| Current Stage 03 GitHub/Codex gate | mixed; correction active | Stage 03 Gate 6 | CR-03-020 was resolved on head `dfe38f2ecfd600bed1a38f8ad21ce9305fc5ab79` by CI PASS and Codex no-major, and GPT Pro accepted that evidence. Closeout head `14145ffb0b2c4fa6f94530f39efb779edbf3e84c` passed CI; PR #9 returned CR-03-028, and replacement PR #10 returned same-head no-major. | Stage 03 planning closeout cannot be marked clean from PR #9 alone; implementation remains blocked until a separate `/goal` | record PR #10 as the method-switch route, correct stale current-state evidence, run local checks, push, request external recheck, and avoid another self-validating evidence commit unless a new finding requires it | no |
