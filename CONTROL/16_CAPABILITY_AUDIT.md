...
| Chrome extension | available | GPT Pro page with login state | none for Stage 00 after successful submission | GPT Pro review completed | use only with user approval | yes |
...
| GitHub connection | unavailable | repo, PR, review | no repo and no auth | Gate 6 blocked | manual GitHub steps | yes |
| GitHub CLI | requires user action | branch, push, PR | `gh` not logged in | cannot create PR | `gh auth login` then manual commands | yes |
| GitHub Actions | unavailable | CI gate | no GitHub repo/PR | CI evidence blocked | workflow files plus manual steps | yes |
| PR `@codex review` | unavailable | Codex review gate | no PR | Gate 6 blocked | manual PR comment after PR exists | yes |
