...
| Chrome extension | degraded but usable with recovery | GPT Pro page and Codex settings pages with login state | direct extension tab creation/navigation timed out on 2026-05-26, but Chrome visible session opened the GPT Pro page | GPT Pro gate can proceed only when visual state is verified and no login/secret prompt appears | use visible Chrome plus local visual/keyboard recovery with screenshots; stop on permission/account prompts | yes |
| Computer Use | available through local visual/keyboard recovery; standalone app tool not exposed | last-resort UI operation | official Computer Use MCP surface not exposed, but local screenshot and keyboard/mouse automation worked for GPT Pro submission | can recover Chrome GPT Pro interactions, but must record screenshots and avoid secrets | Chrome extension first; local visual recovery second; user intervention on login/captcha/payment/permission | yes |
...
| PR `@codex review` | available | Codex review gate | current-head response can lag after comments, requiring bounded retry plus method switch | Stage 01 current head `5bc977b` received no-major response after plugin-route trigger | use CLI comment, minimal retry, GitHub plugin route, then blocker/replacement only if still unresolved | no |
...
| Docker | available for daemon, compose config, and local runtime smoke | Stage 01 scaffold runtime checks | daemon resolved on 2026-05-26: `docker info`, `docker version`, and `docker compose version` pass; `docker compose config` passed after the approved compose file was created; `docker compose up --build -d` and health smoke checks passed locally | Docker no longer blocks local Stage 01 scaffold verification; final acceptance still waits for PR #7 current-head CI/Codex and GPT Pro final review | CI now includes compose config and runtime smoke; keep Docker artifacts out of committed runtime state | no |
...
| GPT Pro page accessibility | available after user instruction | blocking review gate | Chrome extension direct tab control degraded on 2026-05-26; visible Chrome and local visual recovery succeeded | GPT Pro returned Stage 01 implementation-gate CONDITIONAL PASS and permitted implementation-preflight; final implementation review still pending | save response/action items/final result; avoid committing raw browser screenshots or clipboard captures that may include unrelated session context; stop on login/captcha/payment/permission prompts | yes |
