...
| Capability | status | use in project | blocker | impact | fallback | requires user authorization |
| --- | --- | --- | --- | --- | --- | --- |
| Chrome extension background tab route | degraded but usable for tab discovery and partial submission | GPT Pro review without foreground Chrome control | response interaction timed out during source-enabled ChatGPT generation | can submit/capture some evidence, but should not be used in a polling loop | bounded retry once, then record blocker or wait for a clean background route | no unless permission/login prompt appears |
| Standalone background Computer Use | unavailable in current tool surface | user-requested background visual recovery | only foreground screenshot/click tools are locally available; foreground recovery interferes with the user's active Chrome work | cannot satisfy a background-only Computer Use requirement through pyautogui; foreground visual route suspended | use Chrome extension/background control only, or wait for user to provide an idle foreground window/background Computer Use surface | yes for any foreground-only fallback |
