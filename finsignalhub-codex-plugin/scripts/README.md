...
- `phase_check.py`: validates required governance artifacts, rejects unknown stage ids, requires Stage 00.1 subagent evidence, requires explicit plan test categories for Stage 00.1 and later stages, and recursively blocks forbidden Stage 00/00.1 runtime scaffold paths while ignoring local environment, cache, and build directories.
